...
object ActionCacheTest extends BasicTestSuite:
  test("In-memory cache can hold a blob"):
    withInMemoryCache(testHoldBlob)
  ...
  test("In-memory cache can hold action value with blob"):
    withInMemoryCache(testActionCacheWithBlob)
